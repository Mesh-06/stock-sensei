 ...
 * Accepts prefilledTicker and ohlcv data from the parent page.
 * Passes OHLCV directly to Railway so it never needs to call yfinance.
 */
...
import { useStockAnalysis, OHLCVPoint } from "@/hooks/useStockAnalysis";
...
function ForecastChart({
  lastClose, lastDate, predictions,
}: {
  lastClose: number;
  lastDate: string;
  ...
  const data = [
    ...
    ...Object.entries(predictions).map(([date, price]) => ({ date, price, type: "forecast" })),
  ];
  ...
  const CustomDot = (props: Record<string, unknown>) => {
    ...
    return <circle cx={cx} cy={cy} r={5} fill="#38BDF8" stroke="#0F172A" strokeWidth={2} />;
  };
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
        ...
        <YAxis domain={[minP, maxP]} tick={{ fontSize: 11, fill: "#94A3B8" }}
          tickFormatter={(v) => fmt(v, 0)} width={64} />
        <Tooltip
        ...
        />
        <ReferenceLine x={lastDate} stroke="rgba(255,255,255,0.2)" strokeDasharray="4 2"
          label={{ value: "Today", fill: "#94A3B8", fontSize: 10 }} />
        <Line type="monotone" dataKey="price" stroke="#38BDF8" strokeWidth={2.5}
          dot={<CustomDot />} activeDot={{ r: 6, fill: "#38BDF8" }} />
      </LineChart>
    </ResponsiveContainer>
  );
}
...
export function StockAnalysisPanel({ prefilledTicker = "", ohlcv }: StockAnalysisPanelProps) {
  ...
  return (
    <div className="space-y-6 py-4">
      <div className="flex gap-2 items-center">
        ...
        />
        <Button onClick={handleAnalyze} disabled={state.status === "loading" || !ticker.trim()} className="gap-2">
      ...
      {state.status === "loading" && (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-10 flex flex-col items-center gap-4 text-muted-foreground">
          ...
          <div className="text-center">
            <p className="font-semibold text-foreground">Running AI analysis for {state.ticker}…</p>
            <p className="text-sm mt-1">
              First-time stocks need ~3–5 minutes to fine-tune the TGT model.<br />
          ...
          <div className="space-y-4 animate-in fade-in duration-500">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-3">
                ...
                <Badge variant="outline" className={isUp
              ...
              <span className="text-xs text-muted-foreground">
                as of {r.last_date} · {new Date(r.generated_at).toLocaleTimeString()}
              </span>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <MetricCard label="Last Close" value={`₹ ${fmt(r.last_close)}`} />
              <MetricCard label="Day 1 Forecast" value={`₹ ${fmt(r.predicted_prices[0])}`} />
              <MetricCard label="Day 1 Change"
                value={`${r.change_pct_day1 >= 0 ? "+" : ""}${fmt(r.change_pct_day1)}%`}
                hint="Predicted % change from last close to first forecast day" />
              <MetricCard label="5-Day Target" value={`₹ ${fmt(r.predicted_prices[4])}`} />
            </div>
            ...
            <Card className="bg-white/5 border-white/10">
              ...
              <CardContent>
                <ForecastChart lastClose={r.last_close} lastDate={r.last_date} predictions={r.predictions} />
              </CardContent>
            </Card>
                  ...
                  {Object.entries(r.predictions).map(([date, price], i) => {
                    ...
                    return (
                      <div key={date} className="flex items-center justify-between py-2 text-sm">
                        <span className="text-muted-foreground">Day {i + 1} · {date}</span>
                        <div className="flex items-center gap-4">
                          <span className={pctClass(chg)}>{chg >= 0 ? "+" : ""}{fmt(chg)}%</span>
                          <span className="font-semibold tabular-nums w-24 text-right">₹ {fmt(price)}</span>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  <CardContent className="space-y-2">
                    {r.metrics.DirectionalAccuracy !== undefined && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-sky-400">{fmt(r.metrics.DirectionalAccuracy)}%</span>
                      </div>
                    )}
                    {r.metrics.MAPE !== undefined && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">MAPE</span>
                        <span className="font-semibold">{fmt(r.metrics.MAPE)}%</span>
                      </div>
                    )}
              ...
              <Card className="bg-white/5 border-white/10">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {r.peers_used.map((p) => (
                      <Badge key={p} variant="outline" className="text-xs border-white/10 text-muted-foreground">{p}</Badge>
                    ))}
                  </div>
                </CardContent>
              </Card>
            ...
            <p className="text-xs text-muted-foreground opacity-60 leading-relaxed">
              ⚠️ AI model predictions are for educational purposes only and do not constitute financial advice.
      ...
      {state.status === "idle" && (
        <div className="rounded-2xl border border-dashed border-white/10 p-12 text-center text-muted-foreground space-y-2">
          ...
          <p className="font-medium">
            {ohlcv
              ? `Click Analyze to run AI forecast for ${prefilledTicker}`
              : "Enter a stock ticker above to run AI analysis"}
          </p>
          <p className="text-sm opacity-60">Supports NSE (.NS), BSE (.BO), NYSE/NASDAQ, and more</p>
        </div>
      )}
    </div>
  );
